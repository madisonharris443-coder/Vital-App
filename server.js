...
// ... end of analyze-attention route
  }
});
...
app.post("/analyze-improving", async function(req, res) {
  try {
    const { metricKey, metricLabel, pct, scans } = req.body;
    const latestScan = scans[scans.length - 1];
    const profile = latestScan ? 
    ...
    const scanHistory = scans.map(function(s, i) {
      return "Scan " + (i+1) + " (" + new Date(s.date).toLocaleDateString() + "): " +
        metricLabel + "=" + (s[metricKey] || "--") +
        ", sleepSignal=" + (s.sleepSignal || "--") +
        ", stressMarkers=" + (s.stressMarkers || "--") +
    ...
    var prompt = "You are VITAL — an AI health analysis system that has reviewed every scan this person has taken. You know their data cold.\n\n" +
      ...
      "If you write something that could apply to anyone, rewrite it until it only applies to this person. Use scientific terminology when it adds real precision — but always state the plain English meaning first and let the science term follow as the explanation, never the other way around.\n\n" +
      "HEALTH PROFILE:\n" + profile + "\n\n" +
      "METRIC: " + metricLabel + "\n" +
      "IMPROVEMENT: " + pct + "% across " + scans.length + " scans\n\n" +
      "COMPLETE SCAN HISTORY:\n" + scanHistory + "\n\n" +
      "1. WHAT IS IMPROVING (2-3 sentences)\n" +
      "State the exact starting value, current value, and rate of improvement. Note whether it is accelerating or holding steady. Name 1 other metric from their history that is improving in lockstep.\n\n" +
      "2. FACE ZONES (2 zones)\n" +
      "Name 2 anatomical zones where this improvement is visibly showing up. One sentence each — describe what is actually looking better right now.\n\n" +
      "3. HOW TO KEEP MOMENTUM (3 steps)\n" +
      "Give 3 specific actions most likely driving this improvement. Each step: sharp title, 2 sentences. Sentence 1 is what to keep doing with a specific number. Sentence 2 is what happens to this metric if they stop — give a specific reversal timeline.\n\n" +
      "4. POSITIVES\n" +
      "One sentence on the most likely driver. One sentence on exactly what will reverse it.\n\n" +
      "RESPOND ONLY WITH RAW JSON. NO MARKDOWN. NO BACKTICKS. NO PREAMBLE:\n" +
      "{" +
        "\"what\":\"2-3 sentences — exact numbers, rate, lockstep metric\"," +
        "\"zones\":[" +
          "{\"label\":\"Anatomical zone\",\"detail\":\"One sentence on what is visibly better right now\",\"color\":\"good\"}," +
          "{\"label\":\"Anatomical zone\",\"detail\":\"One sentence\",\"color\":\"good\"}" +
        "]," +
        "\"steps\":[" +
          "{\"title\":\"Sharp specific title\",\"detail\":\"Sentence 1: what to keep doing. Sentence 2: reversal timeline if they stop.\"}," +
          "{\"title\":\"Sharp specific title\",\"detail\":\"Sentence 1: keep doing. Sentence 2: reversal timeline.\"}," +
          "{\"title\":\"Sharp specific title\",\"detail\":\"Sentence 1: keep doing. Sentence 2: reversal timeline.\"}" +
        "]," +
        "\"positives\":[" +
          "{\"metric\":\"" + metricLabel + "\",\"trend\":\"exact numbers e.g. improved from 64 to 78 across 22 scans\",\"detail\":\"One sentence on the most likely driver\",\"maintain\":\"One sentence on exactly what will reverse this\"}" +
        "]" +
      "}";
