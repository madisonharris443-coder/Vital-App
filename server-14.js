...
app.post("/save-scans", async function(req, res) {
  ...
  if (session && SUPABASE_SERVICE_KEY) {
    ...
    console.log("No session or service key — skipping Supabase save");
  }
  res.json({ success: true });
});
...
app.get("/get-data", function(req, res) {
  ...
  res.json({ user: user, session: cookies.vital_session || null, scans: "[]", avatar: avatar });
});
...
app.post("/analyze", upload.single("photo"), async function(req, res) {
  try {
    ...
    var p20 = "{\"biologicalAge\":25,\"chronologicalAgeDiff\":\"older by 3 years\",\"agingVelocity\":\"faster than average\",\"agingRate\":\"1.3x faster than baseline\",\"skinHealth\":\"71/100\",\"hydration\":\"65%\",\"inflammation\":\"mild\",\"sleepSignal\":\"deprived\",\"oilBalance\":\"combination-oily T-zone\",\"collagenScore\":\"73/100\",\"stressMarkers\":\"moderate-high\",\"faceSymmetry\":\"84/100\",\"diseaseRisk\":{\"metabolic\":\"34% [82% confidence]\",\"cardiovascular\":\"18% [74% confidence]\",\"inflammation\":\"42% [88% confidence]\",\"hormonal\":\"29% [79% confidence]\"},\"topInsights\":[\"insight 1\",\"insight 2\",\"insight 3\",\"insight 4\"],\"positives\":[\"positive 1\",\"positive 2\",\"positive 3\"],\"recommendations\":[\"[CRITICAL] Organ: action. Why.\",\"[HIGH] Organ: action. Why.\",\"[HIGH] Organ: action. Why.\",\"[MODERATE] Organ: action. Why.\",\"[MODERATE] Organ: action. Why.\"]}\n\n";
